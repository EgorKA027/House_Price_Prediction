...
Чтобы оставить отзыв введите /feedback""")
...
*Аналитика*
Если вас интересует аналитика и визуальная часть данных, которые были собраны для обучения модели, которая используется в данном проекте, нажнимет /analytics 
Аналитика и визуализация данных была сделана автором самостоятельно.             
...
Ваша обратная связь очень важна. Не стесняйтесь делиться этим ботом с друзьями и коллегами для большего охвата аудиторий и получения большего количества оценок.
Автору это очень важно!
Для оставления отзыва - нажмите /feedback""")
...
def handle_analytics_start(message):
    links = 'https://datalens.yandex/o46k6jebtd8qd'
    bot.send_message(message.chat.id, f"""Посмотреть аналитику по данным, которые были собраны для обучения модели можно перейд по ссылке:{links}""")
